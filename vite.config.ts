...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    build: {
      ...
      // Emscripten output needs modern module semantics; esnext unlocks
      // top-level await + native WebAssembly integration for the build.
      target: 'esnext',
    },
    plugins: [
      // Native WASM pipeline for direct `.wasm` module imports. (Vite 8 is
      // Rolldown-based, so the rollup-only `vite-plugin-top-level-await` is
      // incompatible; `target: 'esnext'` keeps top-level await natively, and
      // the Olm engine's binary is fetched at runtime via a fixed public asset.)
      wasm(),
      react(),
      tailwindcss(),
